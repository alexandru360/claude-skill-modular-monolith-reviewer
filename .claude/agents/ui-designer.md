...
## Critical Rule: Verify Design Before Choosing Components

ALWAYS look at the actual screenshot/design visual BEFORE proposing a component solution. Never choose components based on a textual description or diff list alone — the visual design is the source of truth. If a screenshot is available, analyze it first. If not, request one with `get_screenshot`.
